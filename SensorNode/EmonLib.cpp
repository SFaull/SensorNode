/*
  ...
  Mofified to add current read callback for use with external ADC - Sam Faull 06/05/2020
*/
...
void EnergyMonitor::voltage(unsigned int _inPinV, double _VCAL, double _PHASECAL)
{
  ...
  offsetV = ADC_COUNTS(adcResolution)>>1;
}

void EnergyMonitor::setReadCallback(unsigned int(*callback)())
{
  customCallback = true;
...
void EnergyMonitor::current(unsigned int _inPinI, double _ICAL)
{
  ...
  inPinI = _inPinI;
  ICAL = _ICAL;
  offsetI = ADC_COUNTS(adcResolution)>>1;
}

void EnergyMonitor::setAdcResolution(unsigned int _resolution)
{
...
void EnergyMonitor::calcVI(unsigned int crossings, unsigned int timeout)
{
  ...
  while(st==false)                                   //the while loop...
  {
    ...
    if ((startV < (ADC_COUNTS(adcResolution)*0.55)) && (startV > (ADC_COUNTS(adcResolution)*0.45))) st=true;  //check its within range
  ...
  double V_RATIO = VCAL *((SupplyVoltage/1000.0) / (ADC_COUNTS(adcResolution)));
